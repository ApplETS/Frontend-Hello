...
export default function Login({ searchParams }: { searchParams: { message: string } }) {
	return (
		<div className='animate-in relative flex justify-center items-center rounded-2xl w-screen h-screen'>
			<div
				className={`absolute bottom-4 right-4 w-[141px] h-[79px] bg-[url('../public/ETS.svg')] bg-cover`}
			/>
			<div className='grid justify-items-center content-center bg-accent rounded-2xl mx-6 my-6 w-1/3 h-5/6 '>
				...
				{searchParams?.message && <p className='text-red-900'>{searchParams.message}</p>}
				<div className='flex-1 flex flex-col w-full px-8 sm:max-w-md justify-center gap-2'>
					...
					<div className='flex justify-center'>
						<p className='text-xs text-primary'>Vous avez oublié vos informations?
					...
					<div className='flex justify-center'>
						<p className='text-xs pb-10 text-primary'>Vous n&apos;avez pas de compte?
